...
dentus.config(function ($routeProvider) {
	...
	.when('/admin/applications',{
		...
		templateUrl : 'partials/adminApplications.html'
	})
	.when('/admin/visits',{
...
dentus.controller('login',function  ($scope,$http,$location) {
...
dentus.controller('adminSubscribers',function ($http,$scope,$rootScope) {
	...
	$scope.saveSubscriber = function (id) {
		if(saveStatus === 'New'){
			...
			.success(function (data,success) {
				$scope.newSubscriberInfo.serial_number = data[0].serial_number;
...
dentus.controller('adminApplications',function ($http,$scope) {
	$scope.noData = false;
	$scope.connectError = false;
...
dentus.controller('users',function () {
